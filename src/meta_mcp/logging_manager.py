...
from .models import InstallationLogEntry, InstallationError, InstallationSession, ErrorCategory
...
class InstallationLogManager:
    ...
    def start_session(self, server_name: str, option_name: str, install_command: str) -> str:
        ...
        logger.info(f"Started installation session {session_id} for {server_name}-{option_name}")
        return session_id
    ...
    def end_session(self, success: bool, final_message: str) -> None:
        ...
        logger.info(f"Ended installation session {self.current_session.session_id} - Success: {success}")
    ...
    async def log_installation_attempt(
        ...
        try:
            ...
            success, stdout, stderr, return_code = await self._execute_with_logging(command, cwd)
    ...
    def _analyze_error(self, stderr: str, stdout: str, command: str) -> InstallationError:
        ...
        if "permission denied" in error_text or "eacces" in error_text:
        ...
        elif "not found" in error_text and ("npm" in command_lower or "npx" in command_lower):
            ...
            category = ErrorCategory.DEPENDENCY_MISSING
            suggestion = "Install uv/uvx using: curl -LsSf https://astral.sh/uv/install.sh | sh"
    ...
    def _update_failed_installs_log(self) -> None:
        ...
        try:
            ...
            failed_data[server_key] = {
                ...
                "session_id": self.current_session.session_id,
                "error_categories": [
                    attempt.error.category for attempt in self.current_session.attempts 
                    if attempt.error
                ],
                "final_message": self.current_session.final_message,
            ...
            if self.current_session and not self.current_session.success:
                ...
                for attempt in self.current_session.attempts:
                    if attempt.error:
                        ...
                        summary_data["error_categories"][category] = summary_data["error_categories"].get(category, 0) + 1
                
                # Track server failure rates
                if server_key not in summary_data["server_failure_rates"]:
                    summary_data["server_failure_rates"][server_key] = {"attempts": 0, "failures": 0}
