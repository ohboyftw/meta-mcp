...
from .models import IntegrationResult, MCPClientIntegration, MCPServerOptionEnhanced
...
class MCPIntegrationManager:
    ...
    async def integrate_server(
        ...
        logger.info(
            f"Starting integration of {server_name} with clients: {client_targets}"
        )
        ...
        for client in client_targets:
            ...
            try:
                result = await self._integrate_with_client(
                    server_name, client, integrations[client]
                )
                results.append(result)
                logger.info(
                    f"Integration with {client}: {'✅ Success' if result.success else '❌ Failed'}"
                )
            except Exception as e:
                logger.error(
                    f"Integration with {client} failed: {str(e)}", exc_info=True
                )
                results.append(
                    IntegrationResult(
                        success=False,
    ...
    async def _integrate_with_client(
        ...
        config_path = Path(
            integration_config.config_path.replace("~", str(Path.home()))
        )
        ...
        if config_path.exists():
            try:
                ...
                logger.warning(
                    f"Invalid JSON in {config_path}, starting with empty config: {e}"
                )
                existing_config = {}
            except Exception as e:
    ...
    async def remove_server_integration(
        self, server_name: str, client_targets: List[str] = None
    ) -> List[IntegrationResult]:
        ...
        results = []
        logger.info(
            f"Removing {server_name} integration from clients: {client_targets}"
        )
        ...
        for client in client_targets:
            ...
            try:
                ...
                logger.error(
                    f"Failed to remove {server_name} from {client}: {e}", exc_info=True
                )
                results.append(
                    IntegrationResult(
                        success=False,
    ...
    def get_client_config_paths(self) -> Dict[str, str]:
        ...
        return {client: str(path) for client, path in self.config_paths.items()}
