    ...
    async def add_server(
        ...
        try:
            ...
            server_entry = MCPConfigEntry(
                command=command, args=args, cwd=cwd, env=env_vars
            )
        ...
        try:
            ...
            for server_name, server_config in config.mcpServers.items():
                ...
                if not server_config.command:
                    ...
                    warnings.append(
                        f"Command '{server_config.command}' for server '{server_name}' may not be available"
                    )
                ...
                if server_config.env:
                    for env_var, value in server_config.env.items():
                        if value.startswith("<") and value.endswith(">"):
                            warnings.append(
                                f"Server '{server_name}' has placeholder environment variable: {env_var}"
                            )
                        elif not value.strip():
                            warnings.append(
                                f"Server '{server_name}' has empty environment variable: {env_var}"
                            )
