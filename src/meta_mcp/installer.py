...
class MCPInstaller:
    ...
    async def install_server(
        ...
        session_id = self.log_manager.start_session(
            server_name, option_name, install_command
        )

        try:
            success, message = await self._execute_installation_with_fallback(
                install_command, server_name, option_name
            )
            ...
            if success:
                ...
                if request.auto_configure:
                    ...
                    local_config_updated = await self._update_local_mcp_config(
                        config_name, install_command, request.env_vars
                    )
                ...
                if request.auto_configure:
                    if local_config_updated:
                        config_message = (
                            "Local .mcp.json configuration updated automatically."
                        )
                    else:
                        config_message = (
                ...
                self.log_manager.end_session(
                    True, f"Successfully installed {server_name}. {config_message}"
                )
                ...
                parsed_message = (
                    self._parse_npm_error(message)
                    if (
                        install_command.startswith("npm")
                        or install_command.startswith("npx")
                    )
                    else message
                )
                ...
                self.log_manager.end_session(
                    False, f"Installation failed: {parsed_message}"
                )
    ...
    async def install_server_complete(
        ...
        try:
            ...
            if not server_config:
                results.summary = (
                    f"❌ No integration configuration found for {server_name}"
                )
                return results
            ...
            if results.overall_success:
                restart_needed = any(
                    r.restart_required for r in integration_results if r.success
                )
            ...
            logger.error(
                f"Complete installation failed for {server_name}: {e}", exc_info=True
            )
            results.summary = f"❌ Complete installation failed: {str(e)}"
            return results

    def _get_enhanced_server_config(
        self, server_name: str, option_name: str
    ) -> Optional[MCPServerOptionEnhanced]:
                ...
                if server_name in category_servers:
                    ...
                    if option_name in server_info.get("options", {}):
                        ...
                        if "integrations" in option_data:
                            ...
                            return self._create_basic_integration_config(
                                server_name, option_data
                            )
    ...
    def _create_basic_integration_config(
        self, server_name: str, option_data: Dict
    ) -> MCPServerOptionEnhanced:
        ...
        integrations = {
            "claude_desktop": MCPClientIntegration(
                ...
                config_template={
                    "mcpServers": {
                        server_name: {"command": command, "args": args, "env": {}}
                    }
                },
    ...
    def _get_server_definitions(self) -> Dict[str, Dict]:
        ...
        for config_path in config_locations:
            try:
                ...
                logger.error(
                    f"Failed to parse server definitions config at {config_path}: {e}"
                )
                continue
            except Exception as e:
                logger.error(
                    f"Error loading server definitions from {config_path}: {e}"
                )
                continue

        logger.warning(
            "No server definitions config file found, using empty definitions"
        )
        return {}
    ...
    async def _get_install_command(
        self, server_name: str, option_name: str
    ) -> Optional[str]:
    ...
    async def _execute_installation_with_fallback(
        self, install_command: str, server_name: str, option_name: str
    ) -> Tuple[bool, str]:
        ...
        logger.info(
            f"Primary installation failed for {server_name}, trying fallback methods..."
        )

        # Generate fallback commands
        fallback_commands = await self._generate_fallback_commands(
            server_name, option_name, install_command
        )

        for i, fallback_cmd in enumerate(fallback_commands, 1):
            logger.info(
                f"Trying fallback method {i}/{len(fallback_commands)}: {fallback_cmd}"
            )

            try:
                success, fallback_message, fallback_log_data = (
                    await self.log_manager.log_installation_attempt(
                ...
                if success:
                    ...
                    return (
                        True,
                        f"Installation succeeded using fallback method {i}: {fallback_message}",
                    )
                else:
        ...
        if readme_command:
            ...
            try:
                success, readme_message, readme_log_data = (
                    await self.log_manager.log_installation_attempt(
                ...
                if success:
                    logger.info(
                        f"README-based installation succeeded for {server_name}"
                    )
                    return (
                        True,
                        f"Installation succeeded using README instructions: {readme_message}",
                    )
            except Exception as e:
    ...
    async def _update_claude_config(
        self, config_name: str, install_command: str, env_vars: Optional[Dict[str, str]]
    ) -> None:
        ...
        try:
            ...
            cwd = None
            if not (
                install_command.startswith("npx") or install_command.startswith("uvx")
            ):
    ...
    async def uninstall_server(
        self, server_name: str, remove_config: bool = True
    ) -> bool:
    ...
    async def update_server(self, server_name: str) -> str:
        ...
        try:
            ...
            session_id = self.log_manager.start_session(
                server_name, "update", install_info["install_command"]
            )
            success, message, log_data = (
                await self.log_manager.log_installation_attempt(
            ...
            if success:
                ...
                self._save_installation_log()
                self.log_manager.end_session(
                    True, f"Successfully updated {server_name}"
                )
                return f"Successfully updated {server_name}"
            else:
                self.log_manager.end_session(
                    False, f"Failed to update {server_name}: {message}"
                )
    ...
    async def get_server_documentation(
        self, server_name: str, format_type: str = "markdown"
    ) -> Optional[str]:
    ...
    def _guess_category(self, server_name: str):
        ...
        if any(
            term in name_lower for term in ["search", "brave", "google", "perplexity"]
        ):
    ...
    async def _generate_fallback_commands(
        self, server_name: str, option_name: str, original_command: str
    ) -> List[str]:
        ...
        fallback_mappings = {
            ...
            "gitlab": {
                "uvx_alternatives": [
                    "uvx --from git+https://github.com/zereight/gitlab-mcp gitlab-mcp"
                ],
                "npm_alternatives": [],
            },
            ...
            if original_command.startswith(("npm", "npx")):
                ...
                if "@modelcontextprotocol/server-" in original_command:
                    ...
                    fallback_commands.append(
                        f"uvx --from git+https://github.com/modelcontextprotocol/server-{server_name}"
                    )
                ...
                if (
                    "--from git+" in original_command
                    and "modelcontextprotocol" in original_command
                ):
                    fallback_commands.append(
                        f"npx -y @modelcontextprotocol/server-{server_name}"
                    )

        # Remove duplicates and the original command
        fallback_commands = [
            cmd for cmd in fallback_commands if cmd != original_command
        ]
        return list(
            dict.fromkeys(fallback_commands)
    ...
    async def _update_local_mcp_config(
        self, config_name: str, install_command: str, env_vars: Optional[Dict[str, str]]
    ) -> bool:
        ...
        if install_command.startswith("uvx"):
            ...
            logger.warning(
                "You may need to manually configure the command in .mcp.json"
            )
            return False
